...
def stage_3_setup_configured_devices():
    ...
    stage_title = f'Stage 3 > Tracked Devices > Data Source Configuration'
    log_info_msg(f"* > {EVLOG_IC3_STAGE_HDR}{stage_title}")
...
def stage_4_setup_data_sources():
    ...
    stage_title = f"Stage 4 > Data Sources > Tracked Device Assignment"
    log_info_msg(f"* > {EVLOG_IC3_STAGE_HDR}{stage_title}")
    Gb.EvLog.display_user_message(stage_title)
    broadcast_info_msg(stage_title)
    ...
    try:
        if Gb.use_data_source_ICLOUD:
            _login_PyiCloud_again_if_init_not_completed()
...
def stage_4_setup_data_sources_retry(final_retry=False):
    ...
    Gb.trace_prefix = 'STAGE4'
    stage_title = f"Stage 4 > Data Sources > Retry Apple Acct Assignment"
    log_info_msg(f"* > {EVLOG_IC3_STAGE_HDR}{stage_title}")
    Gb.EvLog.display_user_message(stage_title)
    broadcast_info_msg(stage_title)
    ...
    try:
        ...
        for username in Gb.usernames_setup_error_retry_list:
            _login_PyiCloud_again_if_init_not_completed(retry=True)
...
#------------------------------------------------------------------
def _login_PyiCloud_again_if_init_not_completed(retry=False):
    '''
    ...
    Gb.conf_usernames = [apple_account[CONF_USERNAME] for apple_account in Gb.conf_apple_accounts]
