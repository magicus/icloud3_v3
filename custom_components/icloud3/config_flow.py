    ...
    async def async_step_update_apple_acct(self, user_input=None, errors=None):
        ...
        if action_item == 'log_into_apple_acct':
            ...
            if aa_login_info_changed:
                username_password_valid = await self._async_validate_username_password(
                                                username, password)
...
#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    ...
    async def log_into_icloud_account(self, user_input, verify_login=False, called_from_step_id=None):
        '''
        ...
        # Verify the Apple acct can be logged into - a new account, a new username or the p/w has changed
        # if verify_login is None:
        #     conf_username = self.conf_apple_acct[CONF_USERNAME]
        #     conf_password = self.conf_apple_acct[CONF_PASSWORD]
        #     if (username not in Gb.PyiCloud_by_username
        #             or username != conf_username
...
#--------------------------------------------------------------------
    @staticmethod
    def create_PyiCloudService_config_flow( username, password,
                                            endpoint_suffix,
                                            verify_login=False):
        '''
